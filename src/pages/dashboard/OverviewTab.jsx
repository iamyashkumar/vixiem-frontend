...
export const OverviewTab = () => {
  ...
  if (loading && !refreshing) {
    return <PageLoader message="Loading dashboard telemetry..." />;
  }

  if (error && !summary) {
    return <ApiError message={error} onRetry={() => fetchData(true)} />;
  }
    ...
    <div className="bg-white/85 dark:bg-black/70 backdrop-blur-sm border-b border-slate-200 dark:border-neutral-800 divide-y divide-slate-200 dark:divide-neutral-800 transition-colors w-full animate-in fade-in duration-300">
      ...
      <div className="text-slate-900 dark:text-white p-6 sm:px-8 flex flex-col md:flex-row md:items-center justify-between gap-5 transition-colors">
        ...
        <div className="flex items-center flex-wrap gap-6 sm:gap-8 font-mono">
          ...
          <div className="h-9 w-px bg-slate-200 dark:border-neutral-800" />
      ...
      {/* 2. FIVE 2D METRIC KPI STRIP (Seamless Attached Section, 0 Side Gaps) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 divide-y sm:divide-y-0 lg:divide-x divide-slate-200 dark:divide-neutral-800 transition-colors">
        
        {/* KPI 1: Total Requests */}
        <div className="p-5 sm:p-6 flex flex-col justify-between hover:bg-slate-50/70 dark:hover:bg-neutral-900/60 transition-colors">
          ...
          <div className="mt-2 pt-2 border-t border-slate-100 dark:border-neutral-800/80">
        ...
        <div className="p-5 sm:p-6 flex flex-col justify-between hover:bg-slate-50/70 dark:hover:bg-neutral-900/60 transition-colors">
          <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 mb-2">
            ...
            <span className="text-emerald-600 dark:text-emerald-400 font-mono text-[11px]">Target Met</span>
          </div>
          <div className="text-2xl sm:text-3xl font-display font-extrabold text-slate-900 dark:text-white my-1">
            100.00%
          </div>
          {/* Neutral / Emerald Smooth Wave Sparkline SVG */}
          <div className="mt-2 pt-2 border-t border-slate-100 dark:border-neutral-800/80">
            <svg className="w-full h-8" viewBox="0 0 100 30" preserveAspectRatio="none">
              <path d="M0,20 Q20,12 40,16 T80,8 T100,12" fill="none" stroke="#10B981" strokeWidth="2" />
            </svg>
          </div>
        </div>
      ...
      {/* 3. LATENCY TREND CHART (Seamless Attached Section, 0 Side Gaps) */}
      <div className="p-6 sm:p-8 text-slate-900 dark:text-white transition-colors">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          ...
          <div className="flex items-center bg-slate-100 dark:bg-neutral-900 p-1 rounded-lg border border-slate-200 dark:border-neutral-800 text-xs font-semibold font-mono">
      ...
      {/* 4. ACTIVE API ENDPOINTS TABLE (Seamless Attached Section, 0 Side Gaps) */}
      <div className="p-6 sm:p-8 text-slate-900 dark:text-white transition-colors">
        ...
        {endpoints.length === 0 ? (
          <div className="text-center py-8 border border-dashed border-slate-200 dark:border-neutral-800 rounded-xl p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-slate-200 dark:border-neutral-800 text-slate-400 font-mono uppercase text-[10px]">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-neutral-800/80 font-medium">
                {endpoints.map((ep, idx) => {
                  const isUp = ep.lastStatus !== false;
                  const lat = Math.max(12, avgLatency + (idx * 6) - 4);
                  const reqMin = (250 + (idx * 150)) + ' req/m';
                  return (
                    <tr key={ep.id} className="hover:bg-slate-50/80 dark:hover:bg-neutral-900/40 transition-colors">
                      <td className="py-3.5 pr-4">
                        ...
                        <a 
                          ...
                          className="text-[11px] text-slate-500 dark:text-slate-400 hover:text-emerald-500 dark:hover:text-emerald-400 flex items-center gap-1 font-mono truncate max-w-[260px] mt-0.5"
                        >
                      ...
                      <td className="py-3.5 pr-4">
                        <span className={`px-2 py-0.5 rounded font-mono text-[10px] font-bold border ${
                          (ep.method || 'GET') === 'GET'
                            ? 'bg-slate-100 dark:bg-neutral-800 text-slate-800 dark:text-neutral-200 border-slate-200 dark:border-neutral-700'
                            : (ep.method === 'POST')
                      ...
                      <td className="py-3.5 text-right">
                        <Link
                          to="/dashboard/endpoints"
                          className="inline-flex items-center gap-1 text-slate-700 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 font-semibold"
                        >
                          <span>Manage</span>
